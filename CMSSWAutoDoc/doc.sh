...
function checkError(){
    if [ $(echo $?) -ne 0 ]; then
        ...
        #scp $IOFILE cmsdoxy@cmssdt01.cern.ch:/data/doxygen/CMSSWAutoDoc > /dev/null
        cp $IOFILE /eos/project/c/cmsweb/www/cmssdt/doxygen
        exit 1
    fi
}
...
#scp -r $TMP/$REL cmsdoxy@cmssdt01.cern.ch:/data/doxygen
cd $TMP
zip -r ${REL}.zip $REL
cp -r $REL.zip /eos/project/c/cmsweb/www/cmssdt/doxygen
